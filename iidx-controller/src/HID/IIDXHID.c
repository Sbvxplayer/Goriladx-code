...
case 2: {
    static uint16_t last_hid_value = 0;

    // Only consider it "active HID" if the value changed
    if (output_struct->data != last_hid_value) {
        last_led_update = millis();
        last_hid_value = output_struct->data;
...
void HID_task(void) {
    ...
    if (mouse_data.y != 0) send_m_report = true;
